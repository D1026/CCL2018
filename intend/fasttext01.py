...
x_others = text_test[10000:]
p_others = positive_test[10000:]    # 10 0000 + 15002
...
def shuffle(x, p, y1, y2, z1, z2):
    ...
    return shuffled_x, shuffled_p, shuffled_y1, shuffled_y2, shuffled_z1, shuffled_z2


x_final, p_final, act_final, tar_final, key_final, val_final = shuffle(x_final, p_final, act_final, tar_final, key_final, val_final)
x_pry, p_pry, act_pry, tar_pry, key_pry, val_pry = shuffle(x_preliminary, p_preliminary, act_preliminary, tar_preliminary, key_preliminary, val_preliminary)
...
# --------------------------------, 当前数据集类型 ：list
X_train = x_final[:13334]
P_train = p_final[:13334]
Y1_train = act_final[:13334]
Y2_train = tar_final[:13334]
Z1_train = key_final[:13334]
Z2_train = val_final[:13334]

X_vali = x_final[13334:]
P_vali = p_final[13334:]
Y1_vali = act_final[13334:]
Y2_vali = tar_final[13334:]
Z1_vali = key_final[13334:]
Z2_vali = val_final[13334:]


with open("shuffled_pry.pkl", "wb") as f:
    pickle.dump((x_pry, p_pry, act_pry, tar_pry, key_pry, val_pry), f)

with open("xpy1y2z1z2_train.pkl", "wb") as f:
    pickle.dump((X_train, P_train, Y1_train, Y2_train, Z1_train, Z2_train), f)

with open("xpy1y2z1z2_vali.pkl", "wb") as f:
    pickle.dump((X_vali, P_vali, Y1_vali, Y2_vali, Z1_vali, Z2_vali), f)
...
# model.fit([X_train, P_train, X_train], [Y1_train, Y2_train, Z1_train, Z2_train], batch_size=batch_size, epochs=12,
#            verbose=1, callbacks=[tensorboard], \
#            validation_data=([X_vali, P_vali, X_vali], [Y1_vali, Y2_vali, Z1_vali, Z2_vali]))

model.fit([x_final, p_final, x_final], [act_final, tar_final, key_final, val_final], batch_size=batch_size, epochs=6,
           verbose=1, callbacks=[tensorboard], \
           validation_data=([X_vali, P_vali, X_vali], [Y1_vali, Y2_vali, Z1_vali, Z2_vali]))
